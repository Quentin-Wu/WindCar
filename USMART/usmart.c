...
{
	TimerIntClear(TIMER5_BASE, TIMER_TIMA_TIMEOUT);//清除中断标志位 
	TimerLoadSet(TIMER5_BASE, TIMER_A, 80000*100);//将重装载值设置到最大
	//清空定时器的CNT
	usmart_dev.runtime=0;	
}

//获得runtime时间
//返回值:执行时间,单位:0.1ms,最大延时时间为定时器CNT值的*0.1ms
//需要根据所移植到的MCU的定时器参数进行修改
u32 usmart_get_runtime(void)
{
	usmart_dev.runtime+= TimerValueGet(TIMER5_BASE, TIMER_A)/8000;
...
void Timer5IntHandler(void)
{
	...
	usmart_dev.runtime += 1000; //100ms 
	TimerIntClear(TIMER5_BASE, TIMER_TIMA_TIMEOUT);//清除中断标志位 
}
//使能定时器5,使能中断.
void My_Timer5_Init(uint16_t x)
{		
	SysCtlPeripheralEnable(SYSCTL_PERIPH_TIMER5);	//使能外设
	
  TimerConfigure(TIMER5_BASE, TIMER_CFG_PERIODIC);//周期性计数模式
	TimerLoadSet(TIMER5_BASE, TIMER_A, x*100000);//100ms中断一次
	
	IntEnable(INT_TIMER5A);//在系统层使能定时器中断
	TimerIntEnable(TIMER5_BASE, TIMER_TIMA_TIMEOUT); //允许处理某个定时器的中断请求
	IntPrioritySet(INT_TIMER5A, 3);//设置中断优先级
	
	TimerIntRegister(TIMER5_BASE, TIMER_BOTH, Timer5IntHandler); //注册中断处理函数
  TimerEnable(TIMER5_BASE, TIMER_A);//开启定时器
}
#endif
...
{
#if USMART_ENTIMX_SCAN==1
	My_Timer5_Init(sysclk);//分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
